...
public final class ShareDataServiceImpl extends IShareDataService.Stub {
    ...
    private final ArrayList<ShareListener> mShareListener = new ArrayList<>();
    private final HashMap<String, String> mStringData = new HashMap<>();
    private final HashMap<String, Integer> mIntegerData = new HashMap<>();
    private final HashMap<String, Bundle> mBundleData = new HashMap<>();
    private final HashMap<String, HashMap<IBinder, IShareIntCallback>> mIShareIntCallbackMaps = new HashMap<>();
    private final HashMap<String, HashMap<IBinder, IShareStringCallback>> mIShareStringCallbackMaps = new HashMap<>();
    private final HashMap<String, HashMap<IBinder, IShareBundleCallback>> mIShareBundleCallbackMaps = new HashMap<>();
    private final HandlerThread mHandlerThread = new HandlerThread("hzbhd.ShareDataHandlerThread") { // from class: com.hzbhd.proxy.share.impl.ShareDataServiceImpl.1
        {
            start();
        }
    };
    private final Handler mHandler = new Handler(this.mHandlerThread.getLooper()) { // from class: com.hzbhd.proxy.share.impl.ShareDataServiceImpl.2
        ...
        public void handleMessage(Message message) {
            ...
            if (i == 1) {
                ...
                synchronized (ShareDataServiceImpl.this.mIShareIntCallbackMaps) {
                    Map<IBinder, IShareIntCallback> callbackMap = ShareDataServiceImpl.this.mIShareIntCallbackMaps.get(str);
                    if (callbackMap != null) {
                        for (Map.Entry<IBinder, IShareIntCallback> entry : callbackMap.entrySet()) {
                            try {
                                entry.getValue().onInt(i2, message.arg2);
                            } catch (Exception e) {
                                Log.e("ShareDataServiceImpl", "Error invoking onInt callback", e);
                            }
                        }
                    }
                }
            ...
            if (i == 2) {
                ...
                synchronized (ShareDataServiceImpl.this.mIShareStringCallbackMaps) {
                    Map<IBinder, IShareStringCallback> callbackMap = ShareDataServiceImpl.this.mIShareStringCallbackMaps.get(string);
                    if (callbackMap != null) {
                        for (Map.Entry<IBinder, IShareStringCallback> entry : callbackMap.entrySet()) {
                            try {
                                entry.getValue().onString(string2, message.arg2);
                            } catch (Exception e) {
                                Log.e("ShareDataServiceImpl", "Error invoking onString callback", e);
                            }
                        }
                    }
                }
            ...
            synchronized (ShareDataServiceImpl.this.mIShareBundleCallbackMaps) {
                Map<IBinder, IShareBundleCallback> callbackMap = ShareDataServiceImpl.this.mIShareBundleCallbackMaps.get(string3);
                if (callbackMap != null) {
                    for (Map.Entry<IBinder, IShareBundleCallback> entry : callbackMap.entrySet()) {
                        try {
                            entry.getValue().onBundle(bundle3, message.arg2);
                        } catch (Exception e) {
                            Log.e("ShareDataServiceImpl", "Error invoking onBundle callback", e);
                        }
                    }
                }
            }
    ...
    private class MyShareIntDeathRecipient implements IBinder.DeathRecipient {
        private final IShareIntCallback mDiedCallback;
    ...
    private class MyShareBundleDeathRecipient implements IBinder.DeathRecipient {
        private final IShareBundleCallback mDiedCallback;
        private final String mKey;
    ...
    private class MyShareStringDeathRecipient implements IBinder.DeathRecipient {
        private final IShareStringCallback mDiedCallback;
        private final String mKey;
