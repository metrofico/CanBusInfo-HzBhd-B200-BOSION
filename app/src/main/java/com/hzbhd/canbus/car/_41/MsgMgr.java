...
public class MsgMgr extends AbstractMsgMgr {
    ...
    public void initCommand(Context context) {
        ...
        this.nowTemUnit = SharePreUtil.getStringValue(context2, getUigMgr(context2).KEY_TEM_UNIT, "C");
    }
    ...
    private void set0xD0Info() {
        int[] iArr = this.mCanBusInfoInt;
        if (iArr[2] == 17) {
            if (iArr[3] == 0) {
                runOnUi(new AbstractMsgMgr.CallBackInterface() { // from class: com.hzbhd.canbus.car._41.MsgMgr.1
                    @Override // com.hzbhd.canbus.msg_mgr.AbstractMsgMgr.CallBackInterface
                    public void callback() {
                        Toast.makeText(MsgMgr.this.getActivity(), "SUCCESS", 0).show();
                    }
                });
            } else {
                runOnUi(new AbstractMsgMgr.CallBackInterface() { // from class: com.hzbhd.canbus.car._41.MsgMgr.2
                    @Override // com.hzbhd.canbus.msg_mgr.AbstractMsgMgr.CallBackInterface
                    public void callback() {
                        Toast.makeText(MsgMgr.this.getActivity(), "FAIL", 0).show();
                    }
                });
            }
        }
                ...
                default:
                    switch (i) {
                        case 22:
                            ...
                            break;
                        case 23:
                            if (SharePreUtil.getIntValue(this.mContext, "ID41_K_MUTE_OR_HOME", 0) == 0) {
                                realKeyClick2(3);
                                break;
                            } else {
                                realKeyClick2(52);
    ...
    private boolean getTemUnitState() {
        String str = this.nowTemUnit;
        Context context = this.mContext;
        boolean zEquals = str.equals(SharePreUtil.getStringValue(context, getUigMgr(context).KEY_TEM_UNIT, "C"));
        if (!zEquals) {
            Context context2 = this.mContext;
            this.nowTemUnit = SharePreUtil.getStringValue(context2, getUigMgr(context2).KEY_TEM_UNIT, "C");
        }
    ...
    private void setAirData0x21() {
        ...
        if (getTemUnitState() && isAirMsgRepeat(this.mCanBusInfoByte)) {
            return;
        }
    ...
    private void setVehicleSetupData0x32() {
        if (is0x32DataChange()) {
            ...
            arrayList.add(new SettingUpdateEntity(8, 0, Integer.valueOf(DataHandleUtils.getIntFromByteWithBit(this.mCanBusInfoInt[11], 1, 1))));
            arrayList.add(new SettingUpdateEntity(8, 1, Integer.valueOf(DataHandleUtils.getIntFromByteWithBit(this.mCanBusInfoInt[11], 0, 1))));
            ...
            String strByResId = CommUtil.getStrByResId(this.mContext, DataHandleUtils.getBoolBit1(this.mCanBusInfoInt[8]) ? "_96_range_unit_mile" : "unit_km");
            StringBuilder sbAppend = new StringBuilder().append(DataHandleUtils.getBoolBit0(this.mCanBusInfoInt[8]) ? "-" : "");
            int[] iArr = this.mCanBusInfoInt;
            arrayList.add(new SettingUpdateEntity(7, 0, sbAppend.append((iArr[9] * 256) + iArr[10]).append(" ").append(strByResId).toString()).setValueStr(true));
    ...
    private void setCompassData0xD2() {
        if (is0xD2DataChange()) {
            ...
            arrayList.add(new SettingUpdateEntity(9, 1, strByResId).setValueStr(true));
            arrayList.add(new SettingUpdateEntity(9, 2, string).setValueStr(true).setProgress(Integer.valueOf(string).intValue() - 1));
            arrayList.add(new SettingUpdateEntity(9, 3, ((this.mCanBusInfoInt[3] * 3) / 2.0f) + "°").setValueStr(true));
            updateGeneralSettingData(arrayList);
            updateSettingActivity(null);
        }
    }
    ...
    private void setParkAssistData0x25() {
        if (is0x25DataChange()) {
            ArrayList arrayList = new ArrayList();
            arrayList.add(new SettingUpdateEntity(10, 0, getOpenClose(DataHandleUtils.getBoolBit3(this.mCanBusInfoInt[2]))).setValueStr(true));
            arrayList.add(new SettingUpdateEntity(10, 1, getOpenClose(DataHandleUtils.getBoolBit2(this.mCanBusInfoInt[2]))).setValueStr(true));
            arrayList.add(new SettingUpdateEntity(10, 2, getOpenClose(DataHandleUtils.getBoolBit1(this.mCanBusInfoInt[2]))).setValueStr(true));
            arrayList.add(new SettingUpdateEntity(10, 3, getOpenClose(DataHandleUtils.getBoolBit0(this.mCanBusInfoInt[2]))).setValueStr(true));
            updateGeneralSettingData(arrayList);
            updateSettingActivity(null);
        }
    }
    ...
    private void setCameraStatus0xD1() throws SecurityException {
        if (is0xD1DataChange()) {
            boolean zIsForeground = SystemUtil.isForeground(this.mContext, new String[]{HzbhdComponentName.PreCameraMainActivity.getClassName(), HzbhdComponentName.AuxMainActivity.getClassName()});
            ...
            boolean z = ((1 << (SharePreUtil.getBoolValue(this.mContext, "share_41_front_camera_switch", false) ? 2 : 7)) & this.mCanBusInfoInt[3]) != 0;
    ...
    private String resolveAirTemperature(int i) {
        ...
        if (GeneralAirData.fahrenheit_celsius) {
            Context context = this.mContext;
            if (SharePreUtil.getStringValue(context, getUigMgr(context).KEY_TEM_UNIT, "C").equals("F")) {
                return this.df.format(i) + getTempUnitF(this.mContext);
            }
            return this.df.format((i - 32) * 0.55d) + getTempUnitC(this.mContext);
        }
        Context context2 = this.mContext;
        if (SharePreUtil.getStringValue(context2, getUigMgr(context2).KEY_TEM_UNIT, "C").equals("C")) {
            return this.df.format(i * 0.5f) + getTempUnitC(this.mContext);
        }
        return this.df.format((i * 0.5f * 1.8d) + 32.0d) + getTempUnitF(this.mContext);
    }
    ...
    /* JADX WARN: Type inference failed for: r0v0, types: [com.hzbhd.canbus.car._41.MsgMgr$3] */
    private void sendMediaMsg(final String str, final byte[][] bArr) {
        new Thread() { // from class: com.hzbhd.canbus.car._41.MsgMgr.3
    ...
    /* JADX WARN: Type inference failed for: r0v0, types: [com.hzbhd.canbus.car._41.MsgMgr$4] */
    private void reportID3Info(final ID3[] id3Arr, final boolean z) {
        new Thread() { // from class: com.hzbhd.canbus.car._41.MsgMgr.4
            @Override // java.lang.Thread, java.lang.Runnable
            public void run() {
                super.run();
                try {
    ...
    public void reportID3InfoFinal(int i, String str) {
        try {
            CanbusMsgSender.sendMsg(DataHandleUtils.makeBytesFixedLength(DataHandleUtils.byteMerger(new byte[]{22, -53, (byte) i}, DataHandleUtils.exceptBOMHead(str.getBytes("unicodeLittle"))), 22));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private void requestData() {
        ...
        getTimerUtil().startTimer(new TimerTask() { // from class: com.hzbhd.canbus.car._41.MsgMgr.5
            int i = 0;
