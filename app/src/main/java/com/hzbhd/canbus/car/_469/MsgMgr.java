...
import com.hzbhd.canbus.car._469.info.SystemInfo;
import com.hzbhd.canbus.car._469.speech.ISysRxListener;
import com.hzbhd.canbus.car._469.speech.SRxData;
import com.hzbhd.canbus.car._469.speech.STxData;
import com.hzbhd.canbus.car._469.speech.SysRxSpeechReceiver;
import com.hzbhd.canbus.car._469.speech.SysToSpeechReceiver;
import com.hzbhd.canbus.car._469.util.CycleRequest;
...
public class MsgMgr extends AbstractMsgMgr {
    ...
    public void initCommand(Context context) {
        ...
        intent.setComponent(new ComponentName("com.ms.sanychart", "com.ms.sanychart.MainServices"));
        context.startService(intent);
        this.sharedPreferences = context.getSharedPreferences("_s9_prefs", 0);
        getCmd();
    }
    ...
    private void cycleNotifyVersionName(Context context) {
        ...
        CycleRequest.getInstance().start(0, new ActionCallback() { // from class: com.hzbhd.canbus.car._469.MsgMgr.1
    ...
    public void getCmd() {
        String string = this.sharedPreferences.getString("last_cmd", null);
        if (LogUtil.log5()) {
            LogUtil.d("<get_last_cmd>: " + string);
        }
        OptionAirCmd469.getInstance().setTemp = this.sharedPreferences.getFloat("settingTemp", 0.0f);
        OptionAirCmd469.getInstance().compRunSt = this.sharedPreferences.getInt("compRunSt", 0);
        OptionAirCmd469.getInstance().sysRunCtrl = this.sharedPreferences.getInt("sysRunCtrl", 0);
        OptionAirCmd469.getInstance().defrstTimeCtrl = this.sharedPreferences.getInt("value1", 120);
        OptionAirCmd469.getInstance().defrstStopTempSet = this.sharedPreferences.getFloat("value2", 15.0f);
        OptionAirCmd469.getInstance().defrstStartTempSet = this.sharedPreferences.getFloat("value3", 5.0f);
        OptionAirCmd469.getInstance().defrstTimeSet = this.sharedPreferences.getInt("value4", 10);
        if (LogUtil.log5()) {
            LogUtil.d("<OptionAirCmd469 sharedPreferences>: " + OptionAirCmd469.getInstance().setTemp + OptionAirCmd469.getInstance().defrstStopTempSet);
        }
        OptionAirCmd469.getInstance().tempCtrlBckPoorSet = this.sharedPreferences.getFloat("value5", 2.0f);
        if (string != null) {
            if (LogUtil.log5()) {
                LogUtil.d("<get_last_cmd>: " + Arrays.toString(Base64.decode(string, 0)));
            }
            OptionAirCmd469.getInstance().airCmd = Base64.decode(string, 0);
            CanbusMsgSender.sendMsg(OptionAirCmd469.getInstance().airCmd);
            if (LogUtil.log5()) {
                LogUtil.d("<get_last_cmd>: " + Arrays.toString(OptionAirCmd469.getInstance().airCmd));
                return;
            }
            return;
        }
        CanbusMsgSender.sendMsg(new byte[]{22, 24, -1, -88, -5, 60, 0, 7, 90, 70, 7, 0, 0, 1});
    }
    ...
    */
    private java.lang.String getMeaningStr(int[] r17) {
        ...
        */
        throw new UnsupportedOperationException("Method not decompiled: com.hzbhd.canbus.car._469.MsgMgr.getMeaningStr(int[]):java.lang.String");
    }
    ...
    public void canbusInfoChange(Context context, byte[] bArr) {
        ...
        if (getMsDataType(byteArrayToIntArray) == 419393561) {
            SharedAirData(this.mCanBusInfoInt);
            if (this.handsTag && isAirDataChange()) {
                sendAirInfoToSpeech(bArr);
            }
        } else if (getMsDataType(this.mCanBusInfoInt) == 418322406) {
            showRadarView();
            setRadarVoiceData(context);
            ShareBasicInfo(this.mCanBusInfoInt);
        } else if (getMsDataType(this.mCanBusInfoInt) == 218053139) {
        ...
        int[] byteArrayToIntArray2 = getByteArrayToIntArray(bArr);
        this.mCanBusInfoInt = byteArrayToIntArray2;
        if (getMsDataType(byteArrayToIntArray2) == 419407834) {
            setAir0x18FFA7DA(this.mCanBusInfoInt);
        }
    }

    private void setAir0x18FFA7DA(int[] iArr) {
        String str;
        String str2 = null;
        switch (DataHandleUtils.getIntFromByteWithBit(iArr[7], 0, 4)) {
            case 0:
                str = "NoFault";
                break;
            case 1:
                str = "ReturnAirSensorFailure";
                break;
            case 2:
                str = "LowVoltageinCoolingSystemPowerSupply";
                break;
            case 3:
                str = "DefrostSensorFailure";
                break;
            case 4:
                str = "HighVoltageinCoolingSystemPowerSupply";
                break;
            case 5:
                str = "ExhaustSensorFailure";
                break;
            case 6:
                str = "CoolingSystemPressureAlarm";
                break;
            case 7:
                str = "CoolingSystemBackupPowerAlarm";
                break;
            case 8:
        ...
        notifyOtherModule("AIR_FAULT", str);
        int intFromByteWithBit = DataHandleUtils.getIntFromByteWithBit(iArr[8], 0, 2);
        String str3 = "Reserved";
        notifyOtherModule("COMPRESSOR_STATUS", intFromByteWithBit != 0 ? intFromByteWithBit != 1 ? intFromByteWithBit != 2 ? intFromByteWithBit != 3 ? null : "Reserved" : "Defrost" : "Cooling" : "Shutdown");
        notifyOtherModule("AIR_FAN_STATUS", Boolean.valueOf(DataHandleUtils.getIntFromByteWithBit(iArr[8], 2, 2) == 1));
        int intFromByteWithBit2 = DataHandleUtils.getIntFromByteWithBit(iArr[8], 4, 2);
        ...
        notifyOtherModule("POWER_STATUS", str3);
        int intFromByteWithBit3 = DataHandleUtils.getIntFromByteWithBit(iArr[8], 6, 2);
        if (intFromByteWithBit3 == 0) {
            ...
            str2 = "Fault";
        } else if (intFromByteWithBit3 == 3) {
            str2 = "Running";
        }
        notifyOtherModule("SYSTEM_STATUS", str2);
        notifyOtherModule("CABIN_TEMPERATURE", Double.valueOf((DataHandleUtils.getMsbLsbResult(iArr[10], iArr[9]) * 0.1d) - 40.0d));
        notifyOtherModule("DEFROST_TEMPERATURE", Double.valueOf((DataHandleUtils.getMsbLsbResult(iArr[12], iArr[11]) * 0.1d) - 40.0d));
        notifyOtherModule("HIGH_VOLTAGE", Integer.valueOf(DataHandleUtils.getIntFromByteWithBit(iArr[13], 0, 8) * 4));
        notifyOtherModule("COMPRESSOR_CURRENT", Double.valueOf(DataHandleUtils.getIntFromByteWithBit(iArr[14], 0, 8) * 0.1d));
    }
    ...
    private class SanYiVoiceControl implements ISysRxListener {
        @Override // com.hzbhd.canbus.car._469.speech.ISysRxListener
